...
const banCheckMiddleware = async (req, res, next) => {
    const allowedForBannedUsers = [
        ...
        "/support",
        "/support.html",
        "/api/moderation/check-status",
        "/check-session",
        "/logout",
    ];
    const allowedForAnonymous = [
        "/",
        "/signup",
        "/login",
        "/desktop-login",
    ...
    if (!req.session.user) {
        const isAnonymousRoute =
            allowedForAnonymous.includes(req.path) || isStaticAsset;
        if (isAnonymousRoute) {
            return next();
        }
    ...
    db.getActiveBan(req.session.user.username, (err, ban) => {
        ...
        if (ban) {
            const isBannedUserRoute =
                allowedForBannedUsers.includes(req.path) ||
                isSupportAPI ||
                isStaticAsset;
    ...
    `,
        (err, result) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: "Failed to fetch rooms" });
            }
    ...
    `,
        (err, result) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: "Failed to fetch files" });
            }
...
app.get("/api/admin/user-modview/:username", requireAdminAPI, (req, res) => {
    ...
    const queries = {
        user: db.pool.query(
            "SELECT username, email, chat_color, bio, status, avatar_url, role FROM users WHERE username = $1",
            [username],
        ),
        messageCount: db.pool.query(
            "SELECT COUNT(*) as count FROM messages WHERE username = $1",
            [username],
        ),
        fileCount: db.pool.query(
            "SELECT COUNT(*) as count FROM messages WHERE username = $1 AND file_url IS NOT NULL",
            [username],
        ),
        warnings: db.pool.query(
            "SELECT * FROM warnings WHERE username = $1 ORDER BY created_at DESC",
            [username],
        ),
        mutes: db.pool.query(
            "SELECT * FROM mutes WHERE username = $1 ORDER BY created_at DESC LIMIT 10",
            [username],
        ),
        bans: db.pool.query(
            "SELECT * FROM bans WHERE username = $1 ORDER BY created_at DESC LIMIT 10",
            [username],
        ),
        tickets: db.pool.query(
            "SELECT COUNT(*) as count FROM support_tickets WHERE username = $1",
            [username],
        ),
    };
...
app.post("/api/support/tickets", async (req, res) => {
    ...
    try {
        ...
        if (userEmail) {
            const emailSent = await emailService.sendSupportTicketConfirmation(
                userEmail,
                username,
                ticketId,
                subject,
            ...
            if (emailSent) {
                ...
                serverLogger.system("Support ticket confirmation email sent", {
                    ticketId,
                    username,
                    email: userEmail,
                });
        ...
        serverLogger.error("SUPPORT", "Failed to create support ticket", {
            username,
            error: err.message,
        });
...
app.put("/api/support/tickets/:id", requireAdminAPI, async (req, res) => {
    ...
    try {
        const ticketResult = await db.pool.query(
            "SELECT username, email, subject FROM support_tickets WHERE id = $1",
            [id],
        );
        ...
        if (adminResponse && ticket.email) {
            ...
            if (emailSent) {
                ...
                serverLogger.system("Support ticket response email sent", {
                    ticketId: id,
        ...
        serverLogger.error("SUPPORT", "Failed to update support ticket", {
            ticketId: id,
            error: err.message,
        });
    ...
    db.createAnnouncement(title, content, postedBy, (err, announcement) => {
        if (err) {
            ...
            serverLogger.error(
                ...
                { username: postedBy, error: err.message },
            );
            return res
                .status(500)
        ...
        serverLogger.system("Announcement created", {
            title,
            postedBy,
            id: announcement.id,
        });
...
app.delete("/api/admin/announcements/:id", requireAdminAPI, (req, res) => {
    ...
    db.deleteAnnouncement(id, (err, announcement) => {
        ...
        serverLogger.system("Announcement deleted", {
            id,
...
app.patch("/api/admin/announcements/:id/pin", requireAdminAPI, (req, res) => {
    ...
    db.togglePinAnnouncement(id, (err, announcement) => {
        if (err) {
            ...
            serverLogger.error("ANNOUNCEMENTS", "Failed to toggle pin", {
                id,
                error: err.message,
            });
        ...
        serverLogger.system("Announcement pin toggled", {
            id,
...
app.post("/api/announcements/:id/mark-read", (req, res) => {
    ...
    db.markAnnouncementAsRead(username, id, (err) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Failed to mark announcement as read" });
        }
        res.json({ success: true });
    });
});
...
app.get("/api/announcements/unread", (req, res) => {
    ...
    db.getUnreadAnnouncementIds(req.session.user.username, (err, unreadIds) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Failed to fetch unread announcements" });
        }
        res.json({ unreadIds });
    });
});
...
app.get("/api/unread-counts", (req, res) => {
    ...
    db.getUnreadCounts(req.session.user.username, (err, counts) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Failed to fetch unread counts" });
        }
        res.json({ counts });
    });
});
...
app.post("/api/rooms/:room/mark-read", (req, res) => {
    ...
    db.updateRoomReadPosition(username, room, messageId, (err) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Failed to update read position" });
        }
        res.json({ success: true });
    });
});
...
app.get("/api/admin/maintenance", requireAdminAPI, async (req, res) => {
    try {
        ...
        serverLogger.error("SYSTEM", "Failed to get maintenance mode status", {
            error: err.message,
        });
        res.status(500).json({
...
app.post("/api/admin/maintenance", requireAdminAPI, async (req, res) => {
    try {
        ...
        serverLogger.error("SYSTEM", "Failed to toggle maintenance mode", {
            error: err.message,
        });
...
app.get("/api/admin/database-stats", requireAdminAPI, async (req, res) => {
    try {
        ...
        serverLogger.error("DATABASE", "Failed to fetch database statistics", {
            error: err.message,
        });
        res.status(500).json({
...
async function performHealthCheck() {
    try {
        ...
        serverLogger.error("HEALTH", "Health check failed", {
            error: err.message,
        });
    }
}
...
app.get("/api/admin/activity-data", requireAdminAPI, async (req, res) => {
    try {
        ...
        serverLogger.error("DATABASE", "Failed to fetch activity data", {
            error: err.message,
        });
    ...
    db.pool.query(
        ...
        (err, requestsResult) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: "Failed to retrieve stats" });
            }
            ...
            db.pool.query(
                ...
                (err, keysResult) => {
                    ...
                    res.status(200).json({
                        totalRequests:
                            parseInt(requestsResult.rows[0].total) || 0,
...
app.get("/api/developer/keys", (req, res) => {
    ...
    db.getUserApiKeys(req.session.user.username, (err, keys) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Failed to retrieve API keys" });
        }
...
app.post("/api/developer/keys", (req, res) => {
    ...
    if (!appName || appName.trim().length === 0) {
        return res
            .status(400)
            .json({ message: "Application name is required" });
    }
    ...
    db.createApiKey(
        ...
        (err, apiKeyData) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: "Failed to create API key" });
            }
            ...
            res.status(201).json({
                apiKey: apiKeyData.plaintextKey,
    ...
    db.deactivateApiKey(req.session.user.username, keyId, (err, result) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({
                    message: err.message || "Failed to deactivate API key",
                });
        }
...
app.delete("/api/developer/keys/:keyId", (req, res) => {
    ...
    db.deleteApiKey(req.session.user.username, keyId, (err, result) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: err.message || "Failed to delete API key" });
        }
...
app.post("/login", loginLimiter, (req, res) => {
    ...
    console.log(
        `--- SERVER-SIDE LOG: LOGIN from ${clientType.toUpperCase()} client ---`,
    );
...
app.post("/update-profile", (req, res) => {
    ...
    if (display_name && display_name.length > 32) {
        return res
            .status(400)
            .json({ message: "Display name must be 32 characters or less" });
    }
    ...
    db.updateUserProfile(
        ...
        (err) => {
            ...
            if (updatedFields.length > 0) {
                discordWebhook.logProfileUpdate(
                    req.session.user.username,
                    updatedFields,
                );
            }
            ...
            if (status !== undefined) {
                discordWebhook.logStatusChange(
                    req.session.user.username,
                    status,
                );
            }
        ...
        bcrypt.compare(password, user.password, (compareErr, isMatch) => {
            ...
            db.changeUsername(oldUsername, newUsername, (changeErr) => {
                if (changeErr) {
                    ...
                    return res
                        .status(500)
                        .json({ message: "Failed to change username" });
                }
...
app.get("/blocked-users", (req, res) => {
    ...
    db.getBlockedUsers(req.session.user.username, (err, blockedUsers) => {
        if (err) {
            return res
                .status(500)
                .json({ message: "Failed to get blocked users" });
        }
...
app.get("/api/user/stats", async (req, res) => {
    ...
    try {
        ...
        const accountAge = userQuery.rows[0]?.created_at
            ? Math.floor(
                  (Date.now() -
                      new Date(userQuery.rows[0].created_at).getTime()) /
                      (1000 * 60 * 60 * 24),
              )
            : 0;
        ...
        serverLogger.error("USER", "Failed to fetch user stats", {
...
app.get("/api/user/download-data", async (req, res) => {
    ...
    try {
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename="vybez-data-${username}-${Date.now()}.json"`,
        );
        res.send(JSON.stringify(userData, null, 2));
    } catch (error) {
        console.error("Error downloading user data:", error);
        serverLogger.error("USER", "Failed to download user data", {
            username: req.session.user.username,
            error: error.message,
        });
...
app.post("/api/sessions/logout-all", async (req, res) => {
    ...
    try {
        ...
        serverLogger.system("All sessions logged out", {
            username,
        ...
        serverLogger.error("SESSIONS", "Failed to logout all sessions", {
            username: req.session.user.username,
            error: error.message,
        });
...
const emitFullUserList = () => {
    db.getAllUsers((err, allUsers) => {
        ...
        const onlineUsernames = Array.from(onlineUsers.values()).map(
            (u) => u.username,
        );
...
io.on("connection", (socket) => {
    ...
    db.getActiveBan(user.username, (banErr, ban) => {
        if (ban) {
            console.log(
                `Banned user ${user.username} attempted to connect. Disconnecting.`,
            );
            socket.emit("error", {
                type: "banned",
        ...
        console.log(
            `User connected: ${user.username} (${clientType.toUpperCase()})`,
        );
            ...
            db.pool.query(
                ...
                (err, result) => {
                    ...
                    db.deleteMessage(messageId, (deleteErr) => {
                        ...
                        io.to(socket.currentRoom).emit("messageDeleted", {
                            messageId,
                        });
                    });
                },
            ...
            db.getActiveMute(user.username, (muteErr, mute) => {
                ...
                db.isUserBlocked(user.username, to, (blockErr, youBlocked) => {
                    ...
                    db.isBlockedBy(
                        user.username,
                        to,
                        (blockedByErr, blockedByThem) => {
                            if (blockedByErr) {
                                return console.error(
                                    "Error checking if blocked by user:",
                                    blockedByErr,
                                );
                            }
                            if (blockedByThem) {
                            ...
                            db.addPrivateMessage(
                                ...
                                (err, result) => {
                                    ...
                                    discordWebhook.logPrivateMessage(
                                        user.username,
                                        to,
                                        sanitizedText,
                                        clientType,
        ...
        socket.on("markAsRead", ({ messageId }) => {
            ...
            db.addReadReceipt(messageId, user.username, timestamp, (err) => {
                if (err)
                    return console.error("Error adding read receipt:", err);
