...
const searchMessages = (room, query, callback) => { const sql = `SELECT * FROM messages WHERE room = ? AND message_text LIKE ? ORDER BY timestamp DESC LIMIT 50`; db.all(sql, [room, `%${query}%`], (err, rows) => { callback(err, rows); }); };
