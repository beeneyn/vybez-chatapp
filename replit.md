...
-   **Custom Chat Rooms:** User-creatable and deletable rooms with real-time updates. Rooms list now takes full height matching chat/members panels for visual consistency. Room deletion restricted to creator and admins with both frontend and backend permission enforcement. **Unread message indicators** show red badges with message counts next to rooms with unread messages, automatically clearing when the room is viewed.
-   **Real-time Messaging:** Instant messages, history, typing indicators, reactions, @mentions (with notifications), and message deletion. **Smart read tracking** automatically marks messages as read when switching rooms or receiving messages in active rooms. **Admin indicators** display Electric Magenta shield icons next to admin usernames in chat messages for clear role identification.
-   **Discord-Style Members List:** Shows all registered users with online users sorted to the top. Online users have a green status indicator, offline users are dimmed with a gray indicator. Admin users display with a shield icon (Electric Magenta colored) next to their names for easy role identification.
-   **Notifications & Announcements:** In-app notification system with "Mark All Read" button for bulk clearing. Announcements display with unread indicators and individual "Mark as Read" buttons. Announcement badge shows only unread count.
